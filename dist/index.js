...
var setCorsHeaders = (event, config) => {
  ...
  return {
    "Access-Control-Allow-Origin": origin && process.env.NODE_ENV === "production" ? whitelist.includes(origin) ? origin : `https://${config.rootDomain}` : [`http://localhost:${config.localHostPort}`, `https://app.dev.${config.rootDomain}`, ...whitelist].includes(origin) ? origin : `http://localhost:${config.localHostPort}`,
    "Access-Control-Allow-Credentials": true
  };
};

// src/restRequestHandler.ts
var restRequestHandler = (handler) => async (event, _context) => {
  try {
